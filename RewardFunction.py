...
def MelodyRew(key, melodyNote, baseNotes, note_history, step, terminal):
    ...
    if terminal:
        ...
        elif chord.Chord((melodyNote, baseNotes[step])).intervalVector == [0, 0, 0, 0, 0, 0] and chord.Chord((note_history[-1], baseNotes[step - 1])).intervalVector == [0, 0, 0, 0, 0, 0]: # Parallel Octave or Unison
            reward-=1
        
        elif chord.Chord((melodyNote, baseNotes[step])).intervalVector == [0, 0, 0, 1, 0, 0] and chord.Chord((note_history[-1], baseNotes[step - 1])).intervalVector == [0, 0, 0, 1, 0, 0]: # encourage the use of parallel thirds and sixths
            reward+=1

        if chord.Chord((melodyNote, baseNotes[step])).intervalVector in illegal_intervals: # discourage certain intervals between parts
            reward-=1
    ...
    if terminal:
        if chord.Chord((melodyNote, note_history[-1])).commonName == 'Minor Second' or chord.Chord((melodyNote, note_history[-1])).commonName == 'Major Second': # encourage stepwise motion to final note
            reward+=1
    else:
        if chord.Chord((melodyNote, note_history[-1])).commonName in illegal_skips: # discourage the use of illegal intervals
            reward-=1
